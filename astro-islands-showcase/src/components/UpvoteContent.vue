...
<template>
  <div>
    ...
    <div class="m-2 flex items-center rounded-md border border-slate-600 p-2">
      ...
      <button class="h-[80px] w-[80px] rounded-full border border-gray-500 bg-slate-900 p-4 text-center text-sm text-green-600
      hover:scale-90 active:bg-slate-800" @click="upvote">
        <!-- Collapsed svg for brevity -->
        <svg fill="none" class="mx-auto w-8" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"
      ...
      the count is equal to the max count-->
        <div class="mt-2 rounded-md bg-red-300 px-2 text-sm" v-if="upvoteCount === maxUpvoteCount">
          Max upvote reached
        </div>
      </div>
    </div>
